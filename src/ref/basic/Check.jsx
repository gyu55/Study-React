...
const Check = () => {
  ...
  const UnusableId = "test123@gmail.com" || "test456@gmail.com" || "test678@gmail.com"
  const handleCheckButtonOnClick = () => {
    if (inputIdRef.current.value === UnusableId || inputPasswordRef.current.value === ""){
      // setIsUsable(false)
      setResult("검사 실패!")
    }else {
      // setIsUsable(true)
  ...
  return (
    <div>
      <form>
        <div>
          <label>
            <span>아이디</span><input ref={inputIdRef} type="text" />
          </label>
        </div>
        <div>
          <label>
            <span>비밀번호</span><input ref={inputPasswordRef} type="password" />
          </label>
        </div>
        <button button="button" onClick={handleCheckButtonOnClick}>검사!</button>
      </form>
      <div>
